...
class Database:
    ...
    def __open_connection():
        try:
            Database.db =connector.connect(
                option_files=os.path.abspath(os.path.join(os.path.dirname(__file__),"../config.py")),
                autocommit=False
                )
            if "AttributeError" in(str(type(Database.db))):
                raise Exception("foutieve database parameters in config")
            Database.cursor = Database.db.cursor(dictionary=True, buffered=True) # lazy loaded
    ...
    def execute_sql(sqlQuery,params=None):
        ...
        try:
            Database.cursor.execute(sqlQuery, params)
            Database.db.commit()
            result = Database.cursor.lastrowid #bevestigig van create (int of 0)
